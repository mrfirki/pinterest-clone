...
RSpec.describe User, type: :model do
  context "validations" do
	    ...
	    describe "cannot be created without a name" do
		    When(:user) { User.create(email: "josh@nextacademy.com", password: "123456") }
		    Then { user.valid? == false }
	    end

	    describe "cannot be created without a email" do
	      	When(:user) { User.create(username: "Josh Teng", password: "123456") }
	      	Then { user.valid? == false }
	    end
	    ...
	    describe "should permit valid email only" do
	      	...
	      	let(:user2) { User.create(username: "Delilah",email: "delilah.com", password: "123456") }
